...
NAS_FOLDER="/media/NAS_KREBS_1/WGS/DRAGEN_pipeline_results/wgs"
SAMPLES_FILE="wgs_noUHRNA.list"
...
while IFS= read -r sample; do
        ...
        if bcftools sort \
            ...
            bcftools index \
                ...
                -o "step1_index_sort/${sorted_file}.tbi" \
                 >> stepX_log/output.log 2>> stepX_log/error.log || {
    ...
    done < <(find "$folder" -name "*hard-filtered.vcf.gz" -print0 2>/dev/null)
...
# STEP 2: Merge all VCF files
echo "Merging all VCF files"
bcftools merge \
    step1_index_sort/*_sort.vcf.gz \
    --threads 8 \
    -Oz \
    -o step2_merge/merged.vcf.gz \
    >> stepX_log/output.log 2>> stepX_log/error2.log || {
    echo "Merge failed. Check stepX_log/error.log."
    exit 1
}
...
bcftools norm \
    ...
    step2_merge/merged.vcf.gz \
     >> stepX_log/output.log 2>> stepX_log/error3.log || exit 1
...
if ! bcftools index \
    ...
    -o step3_normalize/normalized_merged.vcf.gz.tbi \
     >> stepX_log/output.log 2>> stepX_log/error4.log; then
...
if ! gatk VariantRecalibrator \
    ...
    --dont-run-rscript >> stepX_log/output.log 2>> stepX_log/error5a.log; then
...
if ! gatk --java-options "-Xmx15g -Xms15g" ApplyVQSR \
    ...
    -mode BOTH >> stepX_log/output.log 2>> stepX_log/error5b.log; then
...
if ! gatk SelectVariants \
    ...
    --exclude-filtered >> stepX_log/output.log 2>> stepX_log/error6.log; then
...
gatk --java-options '-Xmx64g' SelectVariants \
    ...
    --output step7_split/filtered_SNV.vcf.gz >> stepX_log/output.log 2>> stepX_log/error7a.log || exit 1
gatk --java-options '-Xmx64g' SelectVariants \
    --variant step6_filtered/filtered.vcf.gz \
    --select-type-to-include INDEL \
    --output step7_split/filtered_INDEL.vcf.gz >> stepX_log/output.log 2>> stepX_log/error7b.log || exit 1
...
gatk --java-options '-Xmx64g' VariantFiltration \
    ...
    --output step8_VAR_Filter/filtered_SNV.filtered.vcf.gz >> stepX_log/output.log 2>> stepX_log/error8a.log || exit 1
...
gatk --java-options '-Xmx64g' VariantFiltration \
    ...
    --output step8_VAR_Filter/filtered_INDEL.filtered.vcf.gz >> stepX_log/output.log 2>> stepX_log/error8b.log || exit 1
...
gatk --java-options '-Xmx64g' SelectVariants \
    ...
    --exclude-filtered >> stepX_log/output.log 2>> stepX_log/error9a.log || exit 1

gatk --java-options '-Xmx64g' SelectVariants \
    -R Homo_sapiens_assembly38.fasta \
    -V step8_VAR_Filter/filtered_INDEL.filtered.vcf.gz \
    -O step9_filtered/filtered_INDEL.filtered.PASS.vcf.gz \
    --exclude-filtered >> stepX_log/output.log 2>> stepX_log/error9b.log || exit 1
...
gatk --java-options '-Xmx64g' MergeVcfs \
    ...
    -O step10_merge/merged.filtered.PASS.vcf.gz >> stepX_log/output.log 2>> stepX_log/error10.log || exit 1

# STEP 11: Apply sed
# Remove 'chr' prefix from chromosome names
# This step uses sed to remove the 'chr' prefix from chromosome names in the VCF file.
zcat step10_merge/merged.filtered.PASS.vcf.gz | sed '/^#/! s/^chr//' > step11_clean/clean.vcf || exit 1
bgzip step11_clean/clean.vcf  # Compress the cleaned file
tabix step11_clean/clean.vcf.gz  # Index it
...
bcftools norm \
    ...
    step11_clean/clean.vcf.gz > stepX_log/step12.log 2> stepX_log/step12.err || exit 1
...
bcftools index \
    ...
    >> stepX_log/output.log 2>> stepX_log/error13a.log || exit 1
...
bcftools annotate \
    ...
    step12_normalize/normalized_merged.filtered.PASS.vcf.gz > stepX_log/step13b.log 2> stepX_log/step13b.err || exit 1  