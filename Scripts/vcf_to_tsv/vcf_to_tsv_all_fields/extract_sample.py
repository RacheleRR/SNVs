...
formatted_data = []
for (chrom, pos, id, ref, alt), values in data.items():
    samples_str = ','.join(values['samples'])
    genotypes_str = ','.join(values['genotypes'])
    formatted_data.append([chrom, pos, id, ref, alt, samples_str, genotypes_str])

# Create a pandas DataFrame and write the data to a file
df = pd.DataFrame(formatted_data, columns=["CHROM", "POS", "ID", "REF", "ALT", "SAMPLES", "GENOTYPES"])
